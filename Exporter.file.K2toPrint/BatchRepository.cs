...
namespace Exporter.file.K2toPrint
{
    internal class BatchRepository
    {
        ...
        public BatchRepository(string conn)
        {
            _connectionString = conn ?? throw new ArgumentNullException(nameof(conn));
        }
        ...
        public async Task<string?> GetBackupFolderAsync()
        {
            const string sql = @"
                SELECT SettingValue
                FROM [JAI_DAILY_EXPORT].[dbo].[Export_PrintSetting]
        ...
        public async Task<Dictionary<string, string>> GetSettingsAsync()
        {
            ...
            await using var cmd = new SqlCommand("SELECT SettingKey, SettingValue FROM dbo.Export_PrintSetting", cn);
        ...
        public async Task InsertOrUpdatePrintSettingsAsync(Dictionary<string, string> settings)
        {
            await using var cn = new SqlConnection(_connectionString);
            await cn.OpenAsync();
            ...
            foreach (var setting in settings)
            {
                await using var cmd = new SqlCommand(@"
                    IF EXISTS (SELECT 1 FROM dbo.Export_PrintSetting WHERE SettingKey = @key)
                        UPDATE dbo.Export_PrintSetting SET SettingValue = @value WHERE SettingKey = @key
                    ELSE
                        INSERT INTO dbo.Export_PrintSetting (SettingKey, SettingValue) VALUES (@key, @value)", cn);
...
ORDER BY JobParamId;";
        ...
        public async Task<long> StartRunAsync(int jobId)
        {
            await using var cn = new SqlConnection(_connectionString);
            await cn.OpenAsync();
            await using var cmd = new SqlCommand("dbo.usp_Export_PrintJobRun_Start", cn)
        ...
        public async Task EndRunAsync(long runId, string status, int? rows, string? file, string? message)
        {
            await using var cn = new SqlConnection(_connectionString);
            await cn.OpenAsync();
            await using var cmd = new SqlCommand("dbo.usp_Export_PrintJobRun_End", cn)
            { CommandType = CommandType.StoredProcedure };
        ...
        public async Task LogErrorAsync(long runId, string stage, string error)
        {
            await using var cn = new SqlConnection(_connectionString);
            await cn.OpenAsync();
            await using var cmd = new SqlCommand("dbo.usp_Export_PrintJobRun_Error", cn)
            { CommandType = CommandType.StoredProcedure };
            cmd.Parameters.AddWithValue("@RunId", runId);
        ...
        public async Task UpdateSourceDataStatusAsync(DataTable ids, int status, string tableName, string pkColumnName)
        {
            ...
            await using var cn = new SqlConnection(_connectionString_repo);
            await cn.OpenAsync();
